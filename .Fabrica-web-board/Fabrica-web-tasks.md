...
_Last recount: 2026-08-29 — Phase 8 closed (45 DONE ✅, 100%) and pushed. Phase 9 starting: WEB-W48 IN_PROGRESS (read-only audit: confirm whether Fabrica-web already exposes the desktop's `/v1/desktop/auth/*` endpoints — PM believes they may already be handled; if confirmed, no separate desktop-routes task needed). WEB-W47 TODO (gated on W48): full `/login` rework per §6 — Google + email/password + recovery + toasts + pair panel, strict §6.0 DNA preservation (dark/copper, server components, next-intl parity en/fr/ar, no new deps, no Orca/Stably, `ai.autoscalers.fabrica` App ID, `fabrica://` deep link, no pre-auth telemetry, no blurred background, RTL Arabic verified, copy grounded in the three marketing files). Also still open: G4-ENV (Supabase GitHub OAuth provider not enabled — backend config)._
...
| Field | Value |
|---|---|
| **Current Group** | Phase 9 — Login rework (W48 audit IN_PROGRESS, W47 TODO gated on W48) |
| **Current Task** | WEB-W48 (read-only audit: confirm whether Fabrica-web already exposes `/v1/desktop/auth/*`); WEB-W47 queued to follow |
| **Last Action** | Phase 8 fully closed + pushed (W38, W39, W40+W40a, W41 all merged/published). W41 doc revised (`b1a20d2`): relay reframed as device auth (no relay login), §6.0 DNA preservation guardrails added (dark/copper, server components, next-intl parity, no new deps, no Orca/Stably, `ai.autoscalers.fabrica` App ID, `fabrica://` deep link, no pre-auth telemetry, no blurred background, RTL Arabic, copy grounded in the three marketing files) + 13-item pre-merge checklist. WEB-W48 dispatched. |
| **Next Action** | Orchestrator: review W48 audit report → confirm/deny the desktop routes → dispatch W47 with the precise spec. Then merge W47 with strict §6.0.3 checklist enforcement. |
...
| `term_7b874684-8b51-48c7-b73c-4e1fa029f6f3` | worker | WEB-W48 read-only desktop auth routes audit | `run_55c168e35a3f / task_127f1e3d5459 / ctx_da1e75043503` | IN_PROGRESS (dispatched) | Aug 29 2026 | web-W48-audit | no |
