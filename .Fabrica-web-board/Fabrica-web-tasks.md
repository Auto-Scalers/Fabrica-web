...
| Field | Value |
|---|---|
| **Current Group** | Phase 9 — Login rework (W48 + W49 + W47 all DONE+MERGED) |
| **Current Task** | none — Phase 9 complete (all 49 tasks DONE ✅, 98%) |
| **Last Action** | W47 merged (`a24888b`): full `/login` rework — legacy-fabrica Supabase wiring mirror (`lib/supabase-browser.ts` singleton + guard; `lib/api/auth.api.ts` re-export wrappers) + rewritten `app/[locale]/login/page.tsx` (three-zone dark/copper card, Google+GitHub+email/password+recovery+toasts+`?intent=web|desktop|pair`+Pair-a-phone panel with inline-SVG QR + `fabrica://pair?token=…&app=ai.autoscalers.fabrica` deep link) + new `app/v1/desktop/auth/invites/route.ts` (Bearer auth, no-`/api` prefix) + new `components/login/{toast-provider.tsx,pairing-code.ts}` + i18n parity en/fr/ar. Build clean (30 routes, 57/57 static); lint 0 new errors / -1 warning vs W40a baseline. W47 rebased onto post-W49 main cleanly. §6.0.3 13-item checklist at `.Fabrica-web-board/WEB-W47-checklist.md` (11 CLEARED, 2 PM follow-ups deferred: #11 marketing copy ground-truth, #13 Vercel Analytics pre-auth on /login). |
| **Next Action** | PM: commit & push Fabrica-web main → Vercel deploy. Verify live: `/login` (default + Google + email + `?recovery` + `?intent=desktop` + `?intent=pair`), `/v1/desktop/auth/invites`, `/v1/artifacts`, `/v1/desktop/auth/{authorize,capabilities,org,profile,refresh,relay-token,session,logout}`. Sign-in 400 should resolve (G4-ENV ✅ applied). |
...
| `term_0d279474-469f-4abc-8235-f069b271cdb0` | worker | WEB-W49 move desktop auth routes /api/v1/* → /v1/* | `run_55c168e35a3f / task_38376ba8ab37 / ctx_fecdbf17441f` | RELEASED (reviewed ✅, merged into main `cf5303c`; worktree to be removed) | Aug 30 2026 | web-W49-prefix | ✅ |
| `term_aa6f5989-9dea-45fe-a55a-d84a0f9a1a98` | worker | WEB-W47 full /login rework (mirror legacy Supabase wiring, dark/copper UI) | `run_55c168e35a3f / task_702f0ac187d5 / ctx_4ddcf14346a9` | RELEASED (reviewed ✅, rebased onto post-W49 main, merged `a24888b`; worktree to be removed) | Aug 30 2026 | web-W47-login | ✅ |
