...
_Last recount: 2026-08-29 — Phase 8 closed (45 DONE ✅) and pushed. **Phase 9 update:** WEB-W48 DONE ✅ — read-only cross-folder audit (385 lines at `.Fabrica-web-board/WEB-W48-desktop-auth-routes-audit.md`) confirmed verdict **(B) ALREADY HANDLED with one caveat**: all 8 desktop auth endpoints + 2 artifact endpoints were already shipped in commit `1bae555` (WEB-CTA worker, 2026-08-29 00:53, ancestor of `main`) under `app/api/v1/desktop/auth/*` + `app/api/v1/artifacts/*`; `lib/fabrica-cloud.ts:mintRelayJwt()` already mints the HS256 relay JWT; the dashboard already calls capabilities + org. **Tracker drift closed:** W41 proposal (13 stale references), W41 row notes ("currently 404"), and the X1 cross-project row (same commit both shipped the routes AND added the TODO claiming they don't exist) were all stale by 22h. X1 row now ✅ DONE. W41 row "open decisions" updated (1, 3, 4, 5 taken; 2 confirmed already-handled). W47 row updated: the 8 `app/v1/desktop/auth/*` "new" caveat dropped (they exist); only the `invites` endpoint remains new (W47-scoped). **One PM action pending (30 seconds):** open `Fabrica-app/src/main/fabrica-profiles/profile-cloud-auth-config.ts:96-118` and confirm whether the URL literal includes `/api` (web ships `/api/v1/desktop/auth/*`; desktop may hard-code `/v1/desktop/auth/*`). If with `/api`, no further action. If without, dispatch a small WEB-W49 (~30 min) to move the 8 web routes to `app/v1/desktop/auth/*` and update the 2 dashboard callers. WEB-W47 (full `/login` rework) remains TODO, dispatched after the prefix check (and any W49) resolves._
...
| WEB-W47 | **Full `/login` rework** per `WEB-W41` proposal §6 + §6.0 (audit-confirmed per WEB-W48: the 8 desktop auth routes already exist on `main` under `app/api/v1/desktop/auth/*`; only the `invites` endpoint is missing and is W47-scoped, not a desktop blocker). **Open PM action:** 30-sec check of `Fabrica-app/src/main/fabrica-profiles/profile-cloud-auth-config.ts:96-118` to resolve the URL-prefix caveat (`/api/v1/...` vs `/v1/...`); if `/v1/...`, dispatch a small WEB-W49 to move the 8 web routes and update the 2 dashboard callers (~30 min). **W47 work is independent of the prefix question** — the page UX is unchanged either way. Scope: (1) Add Google provider (backend already supports it via `app/api/auth/authorize?provider=google`; add the button to the UI). (2) Add email/password form: `supabase.auth.signInWithPassword` + `supabase.auth.signUp` + Sign In / Sign Up pill toggle. (3) Add password recovery: `?recovery=true` + fragment `type=recovery` → new-password form (`supabase.auth.updateUser`); "Forgot password?" → `supabase.auth.resetPasswordForEmail` with `redirectTo: origin + '/<locale>/login?recovery=true'`. (4) Add reusable toast system (bottom-right slide-in 200ms) — replace the full-page red error screen for transient OAuth failures; keep the full state for catastrophic failures. (5) Implement route-driven `?intent=web\|desktop\|pair`: `?intent=desktop` reads `?redirect_to` + `?state` and on success redirects to `redirect_to` instead of `/dashboard`; `?intent=pair` (signed-in) renders the "Pair a phone" panel — QR (inline SVG, no new deps) + copyable 32-char invite code + `fabrica://pair?token=…` deep link. (6) Build the **`app/v1/desktop/auth/invites/route.ts`** (new — the only W47-scoped backend route; build under whichever prefix the PM's check resolves — `/api/v1/desktop/auth/invites` if option (b), or `app/v1/desktop/auth/invites` if option (a)). (7) Three-zone card (header / body / footer) — dark/copper, copper-tinted dark `bg-zinc-900/80 backdrop-blur`, `border:1px solid var(--border-subtle)`, `border-radius:16px`. Header: full-mark logo + "Fab<span class="text-orange-400">.</span>" wordmark + subtitle. Body: Google + GitHub buttons → "or use email auth" divider → email form (Sign In / Sign Up toggle, Email, Password, Forgot Password?) → copper submit. Footer: "← Back to home" left + "New here? Download the app →" right. (8) Loading shell: 40px spinner + copper-tinted border + uppercase 10px copper label ("Signing you in…" / "Connecting…"). (9) i18n parity: all new keys (login.providers.google/github, login.email.*, login.recovery.*, login.pair.*, login.footer.*, login.toast.*, login.loading.*) mirrored across `messages/{en,fr,ar}.json` with identical structure. (10) RTL Arabic layout verified. (11) Strict §6.0 DNA guardrails: dark/copper forge palette (no light surface), server components by default, no new dependencies, no Orca/Stably branding, `ai.autoscalers.fabrica` App ID referenced where the desktop loopback is constructed, `fabrica://` deep link for the pair panel, no pre-auth telemetry (PostHog only after sign-in), copy grounded in the three marketing files in `_sources/` (`brand-guidelines`, `positioning-statement`, `competitor-landscape`). NO blurred dashboard background. NO relay-side login surface. (12) `npm run lint` + `npm run build` clean; all 13 §6.0.3 pre-merge checklist items cleared (attach the checklist as evidence in the worker's worker_done). Update WEB-W47 row → VERIFY + Session Ledger. | ⬜ TODO | Files: `app/[locale]/login/page.tsx` (rewrite), `messages/{en,fr,ar}.json` (new keys), `app/v1/desktop/auth/invites/route.ts` (new). Dispatched after the PM's URL-prefix check + any required WEB-W49 prefix move. |
...
| Field | Value |
|---|---|
| **Current Group** | Phase 9 — Login rework (W48 audit DONE; W47 TODO pending URL-prefix check + any W49) |
| **Current Task** | WEB-W47 (full `/login` rework per §6 + §6.0) — gated on PM's 30-sec check of `Fabrica-app/src/main/fabrica-profiles/profile-cloud-auth-config.ts:96-118` (URL prefix `/api/v1/desktop/auth/*` vs `/v1/desktop/auth/*`) and any required WEB-W49 prefix-move worker |
| **Last Action** | W48 audit confirmed desktop auth routes already shipped on `main` (commit `1bae555`, ancestor of `main`); X1 row reclassified TODO → DONE; W41 / W47 / Rollup / Checkpoint / _Last recount updated to close the 22h tracker drift. |
| **Next Action** | PM: 30-sec check of the desktop file → if `/v1/...` (no `/api`), dispatch WEB-W49 (~30 min, move 8 web routes + update 2 dashboard callers); then dispatch WEB-W47 (the `/login` rework, independent of the prefix). Then merge W47 with strict §6.0.3 13-item checklist enforcement. |
| **Blockers** | G4-ENV: Supabase GitHub OAuth provider not enabled (sign-in 400) — backend config, PM action. PM URL-prefix check pending. |
