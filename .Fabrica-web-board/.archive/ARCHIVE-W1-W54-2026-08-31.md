...
_Last recount: 2026-08-31 — W51 ✅ DONE, W54 ✅ DONE, W50b/W52/W53 ❌ CANCELLED. All 54 tasks resolved. 51 DONE, 3 CANCELLED, 0 TODO._
...
| Field | Value |
|---|---|
| **Current Group** | ALL DONE — 51 DONE, 3 CANCELLED, 0 TODO |
| **Current Task** | None — Fabrica-web complete |
| **Last Action** | W51+W54 completed, W50b+W52+W53 cancelled by PM |
| **Next Action** | PM: commit & push Fabrica-web → Vercel deploy |
| **Blockers** | none |
...
_Last updated: 2026-08-31 (W54 added: dashboard artifacts management)_
...
| WEB-W50b | **Cross-project verification** — Fabrica-app env vars, relay JWT secret, desktop auth flow end-to-end | ❌ Cancelled | PM cancelled: relay/device pairing tasks (W52-W53) dropped. |

---

## Phase 11 — Dashboard Pairing & Relay UX

> `/login?intent=pair` is NOT the right place for web-based pairing. Pairing belongs in the user dashboard — user is already authenticated, sees relay status, and can link/unlink devices. The PairPanel on /login was a shortcut but not the intended UX.

| ID | Task | Status | Notes |
|---|---|---|---|
| WEB-W51 | **Remove `/login?intent=pair` PairPanel from login page.** | ✅ DONE | Removed PairPanel component, `intent='pair'` from Mode type, `formatInviteToken` helper, pairing-code.ts orphaned file deleted. i18n pair.* keys removed from en/fr/ar. Build + lint clean. |
| WEB-W52 | **Dashboard: relay status + manual link/unlink section.** | ❌ Cancelled | PM cancelled: relay/server integration not needed for Beta. |
| WEB-W53 | **Dashboard: auto-link when both desktop and APK are logged in.** | ❌ Cancelled | PM cancelled: relay/server integration not needed for Beta. |
| WEB-W54 | **Dashboard: user artifacts section — full management UI.** Add a dedicated "Artifacts" section to the authenticated `/dashboard` for viewing, managing, sharing, and deleting user artifacts. The existing `/v1/artifacts` API routes (GET list, GET by ID, POST create) are already wired — this task builds the dashboard UI on top of them. Steps: (1) **Artifacts grid/list view** — display all user artifacts (name, type/icon, created date, file size, share status) fetched from `GET /v1/artifacts`. Support both grid (card) and list (table) toggle view. (2) **Search & filter** — client-side search by artifact name; filter by type (document, image, code, other) and date range. (3) **Artifact detail panel** — click an artifact to expand/show detail view: full metadata, preview if applicable (images, code snippets), share link status, and action buttons. (4) **Share management** — for each artifact: generate/copy share link (via existing share API), toggle share on/off, view share URL, copy-to-clipboard button. Show share status indicator (shared/public/private). (5) **Delete flow** — "Delete" button per artifact with confirmation dialog ("Are you sure? This cannot be undone."). Wire to `DELETE /v1/artifacts/[id]` (add endpoint if missing — coordinate with Fabrica-app). (6) **Download** — direct download button per artifact (link to storage URL or proxy download endpoint). (7) **Empty state** — when user has no artifacts: illustration + "No artifacts yet. Create or share artifacts from the Fabrica desktop app." + CTA to download app. (8) **Loading & error states** — skeleton loader during fetch; error toast on API failure; retry button. (9) **Responsive layout** — mobile: stacked card view; desktop: grid with sidebar detail or table view. (10) **i18n parity en/fr/ar** — all new copy keys added to messages/{en,fr,ar}.json under a new `dashboard.artifacts` namespace. (11) `npm run build` + `npm run lint` — 0 new errors. (12) Commit. | ⬜ TODO | Depends on W49 (routes moved to `/v1/artifacts`). Cross-project: may need `DELETE /v1/artifacts/[id]` endpoint if not yet implemented in Fabrica-app; check `app/v1/artifacts/[id]/route.ts` current methods. Also check `lib/supabase.ts` artifact types match UI needs. |
